<template>
    <div class="user_div" v-model="spanbalue" >
        <div v-show="active==0">
            ...
            <div v-if="!blank" class="no_blank"><!-- 调取接口判断用户有没有银行卡 -->
                <div class="addblank" @click="hide(2)" >
                    <img src="../../../assets/img/addblank@2x.png" />
                    增加银行卡
                </div>
                <p>提现请先添加银行卡信息 !</p>
            </div>
            <div class="blank" v-if="blank"><!-- 调取接口判断用户有没有银行卡 -->
        ...
        <div class="adblank" v-if="active==3"> <!-- 绑定银行卡模块 -->
            <div>
                <span class="Fzcolor" @click="hide(0)">提现</span>
                <span class="mar5">></span>
                <span  @click="hide(1)" class="Fzcolor">我的银行卡</span>
                <span class="mar5">></span>
                <span>银行卡详情</span>
            </div>
            <div class="border"></div>
            <div class="blank single">
                <ul >
                    <li class="blank-list no-bg">
                        <img :src="singleBank.bankIconLink" />
                        <div>
                            <p>{{singleBank.bankName}}</p>
                            <p>{{singleBank.bankCard}}</p>
                        </div>
                    </li>
                </ul>
            ...
            <div class="shut">
                <button @click="Unling(singleBank.id)">解除绑定</button>
            </div>
        </div>
        <div class="adblank" v-if="active==2"> <!-- 绑定银行卡模块 -->
            <div>
                <span class="Fzcolor" @click="hide(0)">提现</span>
                <span class="mar5">></span>
                <span  @click="hide(1)" class="Fzcolor">我的银行卡</span>
        ...
        <div class="adblank" v-if="active==1"> <!-- 用户点击我的银行卡查看模块 -->
            ...
            <div class="blank">
                <ul :class="list1?'margin1':''">
                    <li class="blank-list no-bg" v-for="(item,index) in blankdata" :key="index" @click="Unbundling(3,item)">
                        <img :src="item.bankIconLink" />
                        <div>
                            <p>{{item.bankName}}</p>
                            <p>{{item.bankCard}}</p>
...
export default {
  ...
  computed: {
    spanbalue: function() {
      ...
      this.cascadeDisabled = /^([1-9]{1})(\d{14}|\d{18})$/.test(
  ...
  methods: {
    ...
    getmyBankCardList() {
      ...
      axios
        ...
        .then(function(res) {
          ...
          _this.BBlank =
            res.data.result.length > 0 ? res.data.result[0].bankCard : "";
    ...
    Unling(id) {
      ...
      this.$confirm("确定解绑此银行卡吗", "提示", {
        ...
        .then(() => {
          _this.$axios
            .post("/strategist/bindCard/unbundling/" + id)
            .then(res => {
    ...
    userWithd() {
      //用户点击提现;moneyOne payPass BBlank
      var _this = this;
      this.$axios
        .post(
          "/strategist/payment/withdrawals",
          qs.stringify({
    ...
    loadCnaps(provinceCode, cityCode, bankCard) {
      var _this = this;
      axios
        .get("/strategist/cnaps/lists/" + cityCode + "?bankCard=" + bankCard, {
          headers: {
            feign: "true",
            Authorization: this.token
          }
    ...
    loadCities(provinceCode) {
      var _this = this;
      axios
        .get("/strategist/areainfo/children/" + provinceCode)
        .then(function(res) {
          if ("200" == res.data.code) {
    ...
    loadProvinces() {
      var _this = this;
      axios
        .get("/strategist/areainfo/provinces", {
          headers: {
            Authorization: this.token
          }
        })
...
.blank.single p span:first-child {
  display: inline-block;
  width: 110px;
  color: #818081;
}
.blank.single p span:last-child {
  color: #1e242e;
}
.paypassword {
  width: 300px;
